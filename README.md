...
- ### C++ Theory:
   ...
   `std::string` динамически управляет своей памятью, используя техники, подобные динамическому массиву. Под капотом std::string обычно содержит три основных элемента:
    ...
    Когда строка расширяется и текущая вместимость недостаточна для новых данных, `std::string` обычно выделяет новый, бóльший блок памяти, копирует существующие данные в новое место и освобождает старую память.
